...
// In-memory session store for build processes
...
async function ensureEveServer(): Promise<void> {
  // Check if responsive
  try {
  ...
  if (eveProcess) return; // Spawn in progress
...
app.get('/api/build/stream', async (req, res) => {
  ...
  try {
    ...
    const envLines = [];
    if (session.geminiKey) envLines.push(`GEMINI_API_KEY=${session.geminiKey}`);
    else if (process.env.GEMINI_API_KEY) envLines.push(`GEMINI_API_KEY=${process.env.GEMINI_API_KEY}`);
    
    if (session.openaiKey) envLines.push(`OPENAI_API_KEY=${session.openaiKey}`);
    else if (process.env.OPENAI_API_KEY) envLines.push(`OPENAI_API_KEY=${process.env.OPENAI_API_KEY}`);
    
    if (session.token) envLines.push(`GITHUB_TOKEN=${session.token}`);
    else if (process.env.GITHUB_TOKEN) envLines.push(`GITHUB_TOKEN=${process.env.GITHUB_TOKEN}`);

    if (envLines.length > 0) {
      await fs.writeFile(path.join(process.cwd(), '.env.local'), envLines.join('\n') + '\n');
      if (session.geminiKey) process.env.GEMINI_API_KEY = session.geminiKey;
      if (session.openaiKey) process.env.OPENAI_API_KEY = session.openaiKey;
      if (session.token) process.env.GITHUB_TOKEN = session.token;
    }
...
const mockRepoMemory: Record<string, any> = {
  'valibot/valibot': {
    cdds: [
      {
        id: 'cdd-vb-1',
  ...
  if (cleanRepo.includes('precedent')) {
    ...
    try {
      ...
      if (archExists) {
        ...
        const cdds = (architecture.design_decisions || []).map((cdd: any) => ({
        ...
        const checklists = (reviewFailures.failures || []).map((fail: any) => ({
        ...
        const timeline = (previousPrs.prs || []).map((pr: any) => ({
...
function generateMarkdownComment(report: any): string {
...
app.post('/api/github/webhook', async (req, res) => {
  ...
  try {
    ...
    const diffString = diffRes.data as unknown as string;
    ...
    if (report.score === 'BLOCK') conclusion = 'failure';
    else if (report.score === 'FLAG') conclusion = 'neutral';
