...
export async function initCommand(repo: string, options: { token?: string; count?: number }) {
  ...
  await fs.writeFile(
    path.join(userInputDir, "repo-config.json"),
...
export async function buildCommand(options: { token?: string }) {
  const root = process.cwd();
...
export async function updateCommand(options: { token?: string }) {
  const root = process.cwd();
...
export async function reviewCommand(options: { diff?: string }) {
  const root = process.cwd();
...
export async function explainCommand(query: string) {
  const root = process.cwd();
